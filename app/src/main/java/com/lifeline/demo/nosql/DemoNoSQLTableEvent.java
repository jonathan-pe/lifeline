...
public class DemoNoSQLTableEvent extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableEvent.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKeyAndSortKey extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() {
            ...
            result = mapper.load(EventDO.class, AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID(), "demo-name-500000");
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLEventResult(result));
    ...
    public class DemoQueryWithPartitionKeyAndSortKeyCondition extends DemoNoSQLOperationBase {
        ...
        private PaginatedQueryList<EventDO> results;
        private Iterator<EventDO> resultsIterator;
        ...
        @Override
        public boolean executeOperation() {
            final EventDO itemToFind = new EventDO();
            ...
            final DynamoDBQueryExpression<EventDO> queryExpression = new DynamoDBQueryExpression<EventDO>()
                .withHashKeyValues(itemToFind)
                .withRangeKeyCondition("name", rangeKeyCondition)
                .withConsistentRead(false)
                .withLimit(RESULTS_PER_RESULT_GROUP);

            results = mapper.query(EventDO.class, queryExpression);
            if (results != null) {
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<EventDO> results;
        private Iterator<EventDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            results = mapper.scan(EventDO.class, scanExpression);
            if (results != null) {
                resultsIterator = results.iterator();
                if (resultsIterator.hasNext()) {
                    return true;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<EventDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final EventDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLEventResult(item));
            itemsRetrieved++;
    ...
    public void insertSampleData() throws AmazonClientException {
        ...
        final EventDO firstItem = new EventDO();
        ...
        firstItem.setFriendsId(DemoSampleDataGenerator.getSampleNumberSet());
        firstItem.setLocation(
            DemoSampleDataGenerator.getRandomSampleString("location"));
        ...
        final EventDO[] items = new EventDO[SAMPLE_DATA_ENTRIES_PER_INSERT-1];
        for (int count = 0; count < SAMPLE_DATA_ENTRIES_PER_INSERT-1; count++) {
            final EventDO item = new EventDO();
            ...
            item.setFriendsId(DemoSampleDataGenerator.getSampleNumberSet());
            item.setLocation(DemoSampleDataGenerator.getRandomSampleString("location"));
    ...
    public void removeSampleData() throws AmazonClientException {
        ...
        final PaginatedQueryList<EventDO> results = mapper.query(EventDO.class, queryExpression);

        Iterator<EventDO> resultsIterator = results.iterator();
        ...
        final List<EventDO> batchOfItems = new LinkedList<EventDO>();
